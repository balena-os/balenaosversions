...
$(document).ready(function() {
  $.getJSON("config.json", function(data) {
    ...
    _(data.devicetypes).each(function(d) {
      $('table#devices tr:last').after(`<tr><td>${d.name}</td><td class="repo ${d.slug}">checking...</td><td class="staging ${d.slug}">checking...</td><td class="production ${d.slug}">checking...</td></tr>`);
    })
    ...
    $.when(
      ...
      $.get(`https://${config.production}/config`)
        ...
        $("td#osreleasedate").html(
          `<div class="tooltip">${releaseDate.fromNow()}<span class="tooltiptext">${releaseDate.format("dddd, MMMM Do YYYY, h:mm:ss a")}</span></div>`
        );

        var stagingDeviceTypes = stagingAPIResult[0].deviceTypes;
        for (var i = 0; i < config.devicetypes.length; ++i) {
          var key = _.findKey(stagingDeviceTypes, { slug: config.devicetypes[i].slug });
            if (key) {
              ...
              if (version.startsWith(osVersion)) {
                $(`td.staging.${config.devicetypes[i].slug}`).addClass("uptodate")
              } else {
                $(`td.staging.${config.devicetypes[i].slug}`).addClass("outofdate")
              }
            }
        }
        ...
        for (var i = 0; i < config.devicetypes.length; ++i) {
          var key = _.findKey(productionDeviceTypes, { slug: config.devicetypes[i].slug });
            if (key) {
              var buildId = productionDeviceTypes[key].buildId
              $(`td.production.${config.devicetypes[i].slug}`).html(buildId);
              var version = buildId.replace(".prod", "");
              if (version.startsWith(osVersion)) {
                $(`td.production.${config.devicetypes[i].slug}`).addClass("uptodate")
              } else {
                $(`td.production.${config.devicetypes[i].slug}`).addClass("outofdate")
              }
            }
        }
        ...
        Promise.all(promises).then(function(results) {
          _(results).each(function(r) {
            ...
            $(`td.repo.${slug}`).html(`${version} (<div class="tooltip">${date.fromNow()}<span class="tooltiptext">${date.format("dddd, MMMM Do YYYY, h:mm:ss a")}</span></div>)`);
    ...
    //   var doc = jsyaml.load(data);
    //   var releaseDate = moment(doc[0].date);
    //   $("td#osversion").html(doc[0].version);
    //   $("td#osreleasedate").html(
    //     `${releaseDate.format("dddd, MMMM Do YYYY, h:mm:ss a")} (${releaseDate.fromNow()})`
    //   );
    // });
    //
    // $.get(`https://${config.production}/config`).done(function(data) {
    //   var key = _.findKey(data.deviceTypes, { slug: "raspberry-pi2" });
    //   if (key) {
    //     console.log(data.deviceTypes[key]);
    //   }
    // });
